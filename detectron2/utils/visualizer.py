...
class GenericMask:
    ...
    def union(self, generic_mask_):
        self._mask = (self.mask + generic_mask_.mask >= 1).astype("uint8")
...
class Visualizer:
    ...
    def draw_person_instance_predictions(self, predictions, score_thre=0.6, area_thre=1024, inter_thre=196):
        scores = predictions.scores if predictions.has("scores") else None
        classes = predictions.pred_classes.tolist() if predictions.has("pred_classes") else None
        ...
        if masks is not None:
            ...
            for i in range(len(masks)-1):
                for j in range(i+1, len(masks)):
                    if not del_flag[j]:
                        inter = (masks[i].mask + masks[j].mask) == 2
                        if inter.sum() >= inter_thre:
                            masks[i].union(masks[j])
                            del_flag[j] = True
            masks = [masks[i] for i in range(len(masks)) if not del_flag[i] and masks[i].area() >= area_thre]
            print(" new length: ", len(masks))
        ...
        self.overlay_good_person_instances(
            masks=masks,
            alpha=alpha,
        )
        return self.output
    ...
    def overlay_good_person_instances(
            self,
            *,
            masks=None,
            assigned_colors=None,
